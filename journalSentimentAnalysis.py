...
class TestSentimentAnalysis(unittest.TestCase):
    def setUp(self):
        #Test data with sentiment patterns
    ...
    def test_text_sentimentAnalysis_returnsDf(self):
        #Test function returns a DataFrame
        result = text_sentimentAnalysis(self.test_data)
        self.assertIsInstance(result, pd.DataFrame)
        
    def test_text_sentimentAnalysis_addsCol(self):
        #Test all sentiment columns are added
        result = text_sentimentAnalysis(self.test_data)
    ...
    def test_text_sentimentAnalysis_handlesEmString(self):
        #Test empty strings don't break the analysis
        empty_df = pd.DataFrame({'byline': ['', '', '']})
        result = text_sentimentAnalysis(empty_df)
        self.assertIn('sent_comp', result.columns)
        self.assertEqual(len(result), 3)
        
    def test_text_sentimentAnalysis_handlesNone(self):
    ...
    def test_text_sentimentAnalysis_sentClass(self):
        #Test sentiment types are correctly classified
    ...
    def test_key_sentimentAnalysis_sorting(self):
        #Test results are sorted by sent_comp_mean in descending order
        result = key_sentimentAnalysis(self.df_with_sentiment)  
